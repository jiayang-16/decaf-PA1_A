...
%token ','  ';'  '!'  '('  ')'  '['  ']'  '{'  '}'
%token ':'
%token SCOPY SEALED GUARDED_OP VAR
%token ARRAY_INIT ARRAY_JOINT DEFAULT IN FOREACH
...
ForeachStmt		:	FOREACH '(' BoundVariable IN Expr WHILE Expr ')' Stmt
					{
						$$.stmt = new Tree.Foreach($3.bType,$5.expr,$7.expr,$9.stmt,$1.loc);
					}
				|	FOREACH '(' BoundVariable IN Expr ')' Stmt
					{
						$$.stmt = new Tree.Foreach($3.bType,$5.expr,null,$7.stmt,$1.loc);
					}
				;
BoundVariable	:	VAR IDENTIFIER
					{
						$$.bType = new Tree.BoundVariable($2.ident,null,$1.loc);
					}
				|	Type IDENTIFIER
					{
						$$.bType = new Tree.BoundVariable($2.ident,$1.type,$1.loc);
					}
...
LValue          :	Receiver IDENTIFIER
                ...
                |	VAR IDENTIFIER
                	{
                		$$.lvalue = new Tree.VarIdent($2.ident, $1.loc);
                	}                
                ;
...
Expr            :	LValue
                ...
                |	Call
                |	Constant
                | 	Expr ARRAY_INIT Expr
                	{
                		$$.expr = new Tree.Binary(Tree.ARRAYINIT,$1.expr,$3.expr,$2.loc);
                	}
                | 	Expr ARRAY_JOINT Expr
                	{
                		$$.expr = new Tree.Binary(Tree.ARRAYJOINT,$1.expr,$3.expr,$2.loc);
                	}
                |	Expr '[' Expr ':' Expr ']'
                	{
                		$$.expr = new Tree.SubArray($1.expr,$3.expr,$5.expr,$1.loc);
                	}
                |	Expr '[' Expr ']' DEFAULT Expr %prec DEFAULT
                	{
                		$$.expr = new Tree.DynamicQuery($1.expr, $3.expr, $6.expr, $5.loc);
                	}
                |	'[' Expr FOR IDENTIFIER IN Expr ']'
                	{
                		$$.expr = new Tree.ArrayComp($2.expr,$4.ident,$6.expr,$5.loc,null);
                	}
                |	'[' Expr FOR IDENTIFIER IN Expr IF Expr ']'
                	{
                		$$.expr = new Tree.ArrayComp($2.expr,$4.ident,$6.expr,$5.loc,$8.expr);
                	}	
                |	Expr '+' Expr
                	{
...
Constant        :	LITERAL
				...
				| 	ArrayConstant
					{
						$$.expr = new ArrayConstant($1.constList,$1.loc);
					}
                ;
ArrayConstant	: 	'[' ConstListReceiver ']'
					{
						$$.constList = $2.constList;
					}
				;
ConstListReceiver:	ConstList
				|	/* empty */
					{
						$$ = new SemValue();
						$$.constList = new ArrayList<Tree.Expr>();
					}
				;
ConstList		: 	ConstList ',' Constant
					{
						$$.constList.add($3.expr);
					}
				|	Constant
					{
						$$.constList = new ArrayList<Tree.Expr>();
						$$.constList.add($1.expr);
					}
				;
Actuals         :	ExprList
                |	/* empty */
